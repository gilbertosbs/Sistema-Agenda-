...
void Incluir(void)
{
    ...
    while ((cont < TAM) && (op == 's'))
    {
        ...
        gets(max[cont].matricula);
        printf (" Digite o curso: ");
        gets(max[cont].curso);
        printf (" Digite a data de nascimento: ");
        gets(max[cont].nascimento);
...
void Pesquisar(void)
{
    ...
    while (retorno == 1) //o retorno recebe a quantidade de contatos lidos no fread
    {
        if (strcmp(nome, max[i].nome) == 0 ) //strcmp compara as strings das variaveis
        {
            ...
            printf("\n Matricula..........: %s ",max[i].matricula);
            printf("\n Curso..............: %s ",max[i].curso);
            printf("\n Data de nascimento.: %s ",max[i].nascimento);
            printf("\n Endereço...........: %s ",max[i].endereco);
            printf("\n Nota do aluno......: %s ",max[i].nota);
            printf("\n Numero de faltas...: %s ",max[i].falta);
...
void Sobre(void)
{
    printf("\n\n\t\tCADASTRO DE ALUNOS EM LINGUAGEM  C\n\n");
    printf("\tTrabalho para obtenção de nota em estrutura de dados\n\tProf.Marques\n");
    printf("\tGILBERTO PEREIRA - CJ3002454\n");
    getch();//espera que o usuário pressione uma tecla
}
...
void menu(void)
{
    ...
    do
    {
        ...
        printf("\n\n\t\tSistema De Cadastro De Alunos - Gilberto Pereira em C\n");
...
{
    int var;
    system("title Sistema Cadastro De Alunos - Gilberto A. Pereira");
